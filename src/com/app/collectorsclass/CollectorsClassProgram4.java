...
public class CollectorsClassProgram4 {
	public static void main(String[] args) {
		...
		Map<String, Integer> map1 = productList.stream()
				.collect(Collectors.toMap(productNameFunction, quantityFunction, firstProductQuantityBO));
		
		System.out.println("--- ProductName and Quantity map[if productName is duplicate then first comes quantity as value.] ---");
		map1.forEach((k, v)->{
			System.out.println(k+" - "+v);
		});
		
		Map<String, Integer> map2 = productList.stream()
				.collect(Collectors.toMap(productNameFunction, quantityFunction, lastProductQuantityBO));
		
		System.out.println("--- ProductName and Quantity map[if productName is duplicate then last comes quantity as value.] ---");
		map2.forEach((k, v)->{
			System.out.println(k+" - "+v);
		});
		
		Map<String, Double> map3 = productList.stream()
				.collect(Collectors.toMap(productNameFunction, priceFunction, minPriceBO));
		
		System.out.println("--- ProductName and Quantity map[if productName is duplicate then minimum price as value.] ---");
		map3.forEach((k, v)->{
			System.out.println(k+" - "+v);
		});
		
		Map<String, Double> map4 = productList.stream()
				.collect(Collectors.toMap(productNameFunction, priceFunction, maxPriceBO));
		
		System.out.println("--- ProductName and Quantity map[if productName is duplicate then maximum price as value.] ---");
		map4.forEach((k, v)->{
			System.out.println(k+" - "+v);
		});
	}
